import { DirectiveNode, SelectionNode, GraphQLResolveInfo, ArgumentNode } from '../deps.ts'

const argumentToObject = (argumentNode: any, variables: { [key: string]: any }): any => {
...
const convertArgumentsToObj = (argumentsNode: Readonly<ArgumentNode[]>, variables: { [key: string]: any }): { [key: string]: any } => {
...
const convertDirectiveNodeToObj = (directiveNode: Readonly<DirectiveNode>, variables: { [key: string]: any }): { [key: string]: any } => {
...
export const convertDirectivesToObj = (directives: Readonly<DirectiveNode[]>, variables: { [key: string]: any }): { [key: string]: any } => {
    const directiveObj: { [key: string]: any } = {}

    directives.forEach((directive) => {
        directiveObj[directive.name.value] = convertDirectiveNodeToObj(directive, variables)
    })
    return directiveObj
}


export const resolveInfoToTree = (resolveInfo: GraphQLResolveInfo, variables: { [key: string]: any }): {
    [key: string]: any
} => {
    ...
    function traverseFields(nodes: Readonly<SelectionNode[]>): {
        ...
        nodes.forEach((node: SelectionNode) => {
            if (node.kind === 'Field') {
                ...
                tree[fieldName].__directives = convertDirectivesToObj(node.directives ?? [], variables)
                tree[fieldName].__args = convertArgumentsToObj(node.arguments ?? [], variables)
